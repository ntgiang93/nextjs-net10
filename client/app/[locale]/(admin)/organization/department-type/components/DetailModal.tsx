...
export default function DetailModal(props: DetailProps) {
  ...
  const [form, setForm] = useState<DepartmentTypeDto>({ ...defaultDepartmentTypeDto });
  ...
  useEffect(() => {
    if (!isOpen) {
      setForm((prev) => ({ ...prev, ...defaultDepartmentTypeDto }));
      onResetSelected();
    }
  }, [isOpen]);
      ...
      <ModalContent>
        <>
          <ModalHeader className="flex flex-col gap-1">
            {id > 0 ? msg('edit') : msg('add') + ' ' + t('departmentType').toLowerCase()}
          </ModalHeader>
          <ModalBody>
            ...
            {!isFetching && (
              <Form id="detailForm" onSubmit={onSubmit} className={'flex flex-col gap-3'}>
                ...
                <NumberInput
                  ...
                  name="level"
                  minValue={0}
                  placeholder={`${msg('enter')} ${msg('level')}`}
                  value={form.level}
